...
This section will first introduce the gathered data and divide the data into samples. Key-values for each sample is listed i tables. Then the section will test each sample for the normal distribution assumption using D'Agostino and Pearson test and equal variance assumption using Levene's test. When the assumptions in each sample is tested, they are used to answer hypothesis leading to an answer on the research hypothesis listen in the introduction. After each subsection a table containing the a summary of the test results is created. This will hopefully make it easier to get an overview of what analyses is done in this section.   
...
\subsubsection{Normality test summary}\label{sec:normaltest_summary}
	...
	\caption[Summary, normality tests]{Summary of normality tests done in section \ref{sec:normality_results}} \label{tab:normaltest_summary} \\
		  & Sample id & Normally distributed  & Normally distributed after Box-Cox  \\ \hline
		\textit{Total time} & & & \\
...
\begin{longtable}{p{0.5\textwidth}|l|p{0.25\textwidth}}
	...
	Sample id & Obtained p-value & Samples are from populations with equal variances \\ \hline
