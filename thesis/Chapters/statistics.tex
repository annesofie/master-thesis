...
\begin{table}[H]
	...
	\end{tabular}
	\caption[Total time, all participants]{Total time (seconds) spent on each task}
...
\begin{table}[H]
	...
	\end{tabular}
	\caption[Total time, divided by task]{Total time (seconds) divided by task}
...
\begin{table}[H]
	...
	\end{tabular}
	\caption[Total time, divided by task, only experienced]{Total time (seconds) per task for experienced participants, divided by task}
...
\begin{table}[H]
	...
	\end{tabular}
	\caption[Total time, inexperienced per task]{Total time (seconds) per task for inexperienced participants, divided by task}
...
The assumption that sample 1 and sample 2 follows a normal distribution is now accepted and the transformed data can be used in parametric methods. \\[0.5cm]

The calculations and visual inspections of the rest of the 20 samples are located in the appendix. Table \ref{tab:normaltest_summary} show the results of all normal tests performed with D'Agostino and Pearson method. The appendix column in the summary table refers to where in the appendix the calculations are located. 

\newpage

\subsubsection{Normality test summary}\label{sec:normaltest_summary}

	\begin{longtable}{p{0.25\textwidth}|l|p{2cm}|p{0.25\textwidth}|p{0.05\textwidth}}
	\caption[Summary, normality tests]{Summary of normality tests on all 22 samples} \label{tab:normaltest_summary} \\
